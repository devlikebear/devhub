    ...
    sections: [
      {
        title: '사용 방법',
        content:
          'Lorem Ipsum Generator는 레이아웃 테스트와 디자인 목업을 위한 더미 텍스트를 생성하는 도구입니다. 단어, 문장, 단락 단위로 원하는 개수를 설정하여 즉시 생성할 수 있으며, HTML 태그로 래핑하여 웹 개발에 바로 사용할 수 있습니다.',
      },
      {
        title: 'Lorem Ipsum이란?',
        content:
          'Lorem Ipsum은 인쇄 및 타이포그래피 업계에서 사용되는 표준 더미 텍스트입니다. 1500년대부터 사용되어 왔으며, 실제 콘텐츠 없이 레이아웃의 시각적 형태를 미리 확인하는 용도로 사용됩니다.',
      },
    ],
    examples: [
      {
        title: '5개 단락 생성',
        input: '단위: 단락, 개수: 5',
        output: 'Lorem ipsum... (5개 단락)',
        description: '블로그 포스트나 기사 레이아웃 테스트용',
      },
      {
        title: 'HTML 리스트 생성',
        input: '단위: 문장, 개수: 5, 태그: <ul>',
        output: '<ul><li>Lorem ipsum...</li>...</ul>',
        description: '리스트 형태의 더미 콘텐츠 생성',
      },
      {
        title: '100 단어 생성',
        input: '단위: 단어, 개수: 100, 태그: <p>',
        output: '<p>Lorem ipsum... (100 words)</p>',
        description: '짧은 설명문이나 요약문 테스트용',
      },
    ],
    tips: [
      '슬라이더를 사용하여 개수를 빠르게 조정할 수 있습니다',
      '"Lorem ipsum dolor sit amet..." 옵션을 해제하면 랜덤 단어로만 시작합니다',
      'HTML 태그를 선택하면 바로 복사하여 코드에 붙여넣을 수 있습니다',
      '통계 정보를 참고하여 원하는 길이의 텍스트를 생성하세요',
      '<p>, <div> 태그는 단락별로, <ul>, <ol> 태그는 문장별로 래핑됩니다',
      '생성된 텍스트는 실시간으로 업데이트되어 즉시 확인 가능합니다',
    ],
    faqs: [
      {
        question: 'Lorem Ipsum은 언제 사용하나요?',
        answer:
          '웹사이트나 앱의 레이아웃을 디자인할 때, 실제 콘텐츠 없이 텍스트 배치를 확인하기 위해 사용합니다. 타이포그래피, 간격, 가독성 테스트에 유용합니다.',
      },
      {
        question: '왜 의미 없는 라틴어를 사용하나요?',
        answer:
          '의미 있는 텍스트를 사용하면 리뷰어가 내용을 읽게 되어 레이아웃 자체에 집중하기 어렵습니다. Lorem Ipsum은 단어 길이와 문장 구조가 자연스러워 실제 텍스트처럼 보이면서도 내용에 방해받지 않습니다.',
      },
      {
        question: 'HTML 태그는 어떻게 활용하나요?',
        answer:
          '생성된 HTML 코드를 복사하여 HTML/CSS 파일에 직접 붙여넣으면 됩니다. <p> 태그는 일반 단락, <ul>/<ol>은 리스트, <h1>~<h3>는 제목 테스트에 적합합니다.',
      },
      {
        question: '얼마나 많은 텍스트를 생성할 수 있나요?',
        answer:
          '단어는 최대 500개, 문장은 50개, 단락은 20개까지 생성할 수 있습니다. 대부분의 레이아웃 테스트에 충분한 양입니다.',
      },
      {
        question: '생성된 텍스트는 매번 동일한가요?',
        answer:
          '아니요, 시작 문구("Lorem ipsum...")를 제외하고는 단어 풀에서 랜덤하게 선택되므로 매번 다른 텍스트가 생성됩니다. 다양한 길이의 문장과 단어 조합을 테스트할 수 있습니다.',
      },
    ],
  },
};
