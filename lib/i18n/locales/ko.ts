  ...
  tools: {
    ...
    color: {
      ...
      guide: {
        title: '사용 가이드',
        items: [
          '• 지원 형식: HEX (#3b82f6), RGB (rgb(59, 130, 246)), HSL (hsl(217, 91%, 60%))',
          '• 자동 변환: 하나의 형식을 입력하면 다른 형식으로 자동 변환',
          '• 팔레트 생성: 보색, 유사색, 3색 조화, 명도 단계 자동 생성',
          '• 클릭 복사: 각 색상 코드를 클릭하여 클립보드에 복사',
          '• 디자인 활용: UI/UX 디자인, 브랜드 컬러 선정에 유용',
    ...
    url: {
      ...
      guide: {
        title: '사용 가이드',
        items: [
          '• 전체 URL 모드: encodeURI 사용, URL 구조 유지 (/, :, ? 등)',
          '• 쿼리 파라미터 모드: encodeURIComponent 사용, 모든 특수문자 인코딩',
          '• 예약 문자: ; , / ? : @ & = + $ # 등이 % 형식으로 변환',
          '• 디코딩: 퍼센트 인코딩된 문자를 원래 문자로 복원',
          '• 활용: API 요청, URL 공유, 쿼리 파라미터 전달 시 필수',
          '• 프라이버시: 모든 처리는 브라우저에서 진행, 서버 전송 없음',
        ],
      },
    },
...
DevHub에서 빠르게 마크다운 초안을 작성해보세요!`,
    ...
    imagePage: {
      ...
      guide: {
        title: '사용 가이드',
        items: [
          '• 지원 포맷: PNG, JPEG, WebP, GIF → PNG/JPEG/WebP 변환',
          '• 품질 조정: 1-100% 범위로 파일 크기와 품질 균형 조절',
          '• 크기 조정: 너비/높이 지정으로 이미지 리사이즈 (비율 자동 유지)',
          '• 자동 변환: 이미지 업로드 시 즉시 변환 결과 확인',
          '• 다운로드: 변환된 이미지를 클릭하여 다운로드',
          '• 프라이버시: 모든 처리는 브라우저에서 진행, 서버 업로드 없음',
        ],
      },
    },
    loremPage: {
