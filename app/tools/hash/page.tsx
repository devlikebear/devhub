      ...
      <main className="max-w-5xl mx-auto px-6 py-20">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
          <div className="lg:col-span-2 p-6 bg-white/80 dark:bg-gray-800/50 rounded-lg border border-gray-300 dark:border-gray-700">
            <label className="block text-gray-900 dark:text-white font-semibold mb-3" htmlFor="hash-input">
              {text.inputLabel}
            </label>
            <textarea
              id="hash-input"
              ...
              rows={8}
              className="w-full px-4 py-3 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white rounded-lg border border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:outline-none resize-none"
            />
          ...
          <div className="p-6 bg-white/80 dark:bg-gray-800/50 rounded-lg border border-gray-300 dark:border-gray-700">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{text.algorithmLabel}</h2>
                ...
                <label
                  ...
                  className={`flex items-start gap-3 p-3 rounded-lg border transition-colors cursor-pointer ${
                    algorithm === option
                      ...
                      : 'border-gray-300 dark:border-gray-700 hover:border-gray-300 dark:border-gray-600'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  />
                  <div>
                    <p className="text-gray-900 dark:text-white font-medium">{text.algorithms?.[option]?.label || option.toUpperCase()}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">{text.algorithms?.[option]?.description || ''}</p>
                  </div>
                </label>
              ))}
            </div>
          ...
          {result ? (
            <div className="space-y-4">
              ...
              <div className="p-4 bg-gray-100 dark:bg-gray-900 rounded-lg border border-gray-300 dark:border-gray-700">
                <p className="text-gray-600 dark:text-gray-400 text-sm">Output byte length</p>
                <p className="text-gray-900 dark:text-white font-mono text-lg">{result.byteLength} bytes</p>
              </div>
            </div>
          ) : (
            <p className="text-gray-600 dark:text-gray-400 text-sm">{text.emptyMessage}</p>
          )}
        </section>
...
function ResultRow({
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 p-4 bg-gray-100 dark:bg-gray-900 rounded-lg border border-gray-300 dark:border-gray-700">
      <div className="md:w-3/4">
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">{label}</p>
        <p className="text-gray-900 dark:text-white font-mono break-all text-sm md:text-base">{value}</p>
      </div>
      <button
        onClick={onCopy}
        className="self-start md:self-center px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/20 text-gray-900 dark:text-white rounded-lg text-sm transition-colors"
      >
        {copyLabel}
      </button>
    </div>
